...
// svcName is the empty string, the entry is treated as node-level and the
// (Funnel on / tailnet only) annotation is included; when svcName is set,
// the line is prefixed with the service name and no funnel annotation is
// shown (services have no Funnel concept).
//
