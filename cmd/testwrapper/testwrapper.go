...
// per-attempt -timeout. It returns the test's outcome ("pass" / "fail" /
// "skip"), the wall-clock time spent on this attempt (used for the per-test
// retry budget), and any captured test logs.
//
// On panic, timeout, or any other failure mode where the test does not emit a
// pass/fail/skip JSON event, outcome is reported as "fail".
func runOneTest(ctx context.Context, pkg, testName string, perAttemptTimeout time.Duration, attemptNum int, goTestArgs, testArgs []string) (outcome string, wallDur time.Duration, logs bytes.Buffer, err error) {
	...
	if waitErr != nil && err == nil && outcome == "pass" {
		// A non-zero exit when outcome=="pass" is unexpected; surface it.
		err = waitErr
	}
...
func retryFailedTest(ctx context.Context, ft *failedTest, goTestArgs, testArgs []string) {
	...
	for {
		...
		fmt.Printf("    [retry %d] %s.%s: %s (%.3fs)\n",
			ft.attempts, ft.pkg, ft.testName, strings.ToUpper(outcome), dur.Seconds())
		if err != nil {
			log.Printf("testwrapper: error running %s.%s: %v", ft.pkg, ft.testName, err)
		}
		if testingVerbose || outcome == "fail" {
...
func main() {
	...
	printPkgOutcome := func(pkg, outcome string, cached bool, testDur time.Duration) {
		if pkg == "" {
	...
	for _, pkgPattern := range packages {
		...
		for tr := range ch {
			...
			if tr.pkgFinished {
				...
				if tr.outcome == "fail" && len(pkgFailedTests) == 0 {
