...
//
// Marking a test with [Mark] is no longer required for cmd/testwrapper to
// retry failed tests — the wrapper now retries any failure within a per-test
// time budget and reports a test as flaky if it ever passes on retry. Mark is
// still useful for tracking a known-flaky test against a GitHub issue and for
// the [TS_SKIP_FLAKY_TESTS] skip behavior used to keep CI green when a flake
